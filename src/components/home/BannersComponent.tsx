                ...
                {banners.map((banner: any, index: number) => (
                  <SwiperSlide key={index}>
                    <Link className='relative flex flex-col justify-center items-center h-full bg-cover bg-center bg-no-repeat px-10 sm:px-16 lg:px-20'
                      ...
                      {banner.title &&
                        <>
                          <h2 className='md:hidden text-center pb-2 md:max-w-[700px] lg:max-w-[900px] w-full text-white uppercase text-size-title'>{banner.title}</h2>
                          <h2 style={{textAlign: banner.contentPosition}} className='hidden md:block pb-2 md:max-w-[700px] lg:max-w-[900px] w-full text-white uppercase text-size-title'>{banner.title}</h2>
                        </>
                      }
                      {banner.description &&
                        <>
                          <p className='hidden sm:block md:hidden text-size-item text-center md:max-w-[700px] lg:max-w-[900px] w-full text-white '>{banner.description}</p>
                          <p style={{textAlign: banner.contentPosition}} className='hidden md:block md:max-w-[700px] lg:max-w-[900px] w-full text-size-item text-white'>{banner.description}</p>
                        </>
                      }
                    </Link>
                  </SwiperSlide>

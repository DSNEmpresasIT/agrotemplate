  ...
  <div className={`mx-auto relative max-w-[1200px] w-full flex justify-between ${isOpen ? "items-start" : 'items-center'}`}>
    ...
    <div className="flex ps-2 ms-auto justify-end">
      ...
      <ul className={`md:flex flex-col md:w-auto justify-end flex ms-auto w-1/2 md:flex-row ${isOpen ? "flex" : "hidden"} gap-5 items-center`}>
          ...
          <li className="relative">
            ...
            <ul className="focus:flex hidden z-[9999] absolute animate-flip-down animate-ease-linear animate-duration-200 animate-alternate animate-fill-forwards peer-hover:flex hover:flex flex-col min-w-[200px] bg-light">
              <li className="flex border-b-1 border-white hover:bg-white">
                <Link href={`${CUSTOMPATHS.CATALOG}?categoria=Semillas`} className="text-start px-4 py-3 text-white  hover:bg-white hover:text-light" title="Semillas">
                  <span className="hover:translate-x-3">Semillas</span>
                </Link>
              </li>
              <li className="flex border-b-1 border-white hover:bg-white">
                <Link href={`${CUSTOMPATHS.CATALOG}?categoria=Protección de cultivo`} className="text-start px-4 py-3 text-white   hover:bg-white hover:text-light" title="Protección de cultivo">
                  <span className="hover:translate-x-3">Protección de cultivo</span>
                </Link>
              </li>
              <li className="flex border-b-1 border-white hover:bg-white">
                <Link href={`${CUSTOMPATHS.CATALOG}?categoria=Fertilizantes`} className="text-start px-4 py-3 text-white   hover:bg-white hover:text-light" title="Fertilizantes">
                  <span className="hover:translate-x-3">Fertilizantes</span>
                </Link>
              </li>
              <li className="flex hover:bg-white">
                <Link href={`${CUSTOMPATHS.CATALOG}?categoria=Mas insumos agrícolas`} className="text-start px-4 py-3 text-white hover:bg-white hover:text-light" title="Más insumos agrícolas">
                  <span className="hover:translate-x-3">Varios</span>
                </Link>
              </li>
            </ul>
        ...
        <li>
          <Link href={CUSTOMPATHS.NEWS} rel="canonical" className="pb-2 hover:text-light text-white" title="Noticias">
            Noticias
          </Link>
        </li>
        <li>
          <Link href={CUSTOMPATHS.GALLERY} rel="canonical" className="pb-2 hover:text-light text-white" title="Galería">
            Galería
          </Link>
        </li>
        <li>
          <Link href={CUSTOMPATHS.CONTACT} className="pb-2 hover:text-light text-white" title="Contacto">
            Sobre nosotros
          </Link>
        </li>
        {/* <li className="relative">
        ...
        {pathname !== CUSTOMPATHS.HOME && (<li>
          <button onClick={() => toggleCartVisibility()} className=" px-2 flex items-center hover:text-light gap-2 text-white" title="Cotización">
            Cotización
            {cart.length > 0 && <span className="rounded-full bg-light text-white p-1 px-2 flex items-center gap-2">{cart.length} <TbShoppingCartQuestion className="md:text-2xl" /></span>}
          </button>
          <CartComponent />
        </li>)}
      </ul>
      <button className="group text-white flex md:hidden" onClick={() => setIsOpen(!isOpen)}>
        <IoMdMenu className="text-2xl hover:text-light" />
      </button>
    </div>
  </div>

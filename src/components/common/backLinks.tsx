    ...
    <div className='flex flex-wrap'>
      {backlinks.map((enlace, index) => (
        <span key={index} className='text-[#185983] flex items-center justify-center '>
          <Link className={`text-size-item hover:underline ${  index === (backlinks.length - 1) ? 'text-[#185983]/60' : 'text-[#185983]'}`} href={`${enlace.ruta === CUSTOMPATHS.CATALOG ? '' : ''}${enlace.ruta}`}>{enlace.nombre}</Link>
